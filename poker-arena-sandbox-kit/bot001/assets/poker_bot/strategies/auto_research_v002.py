"""Auto-research candidate v002.

This candidate keeps the current champion as the default policy. Compared with
v001, the premium preflop pressure only activates when most observed opponents
show high call frequency, avoiding mixed tables where the v001 gate was too
wide.
"""
...
def preflop_premium_pressure(table, my_seat, base):
    ...
    if hand in PREMIUM_OPEN_CLASSES or score >= 82:
        ...
        return "raise", amount, f"v002 six-max premium open pressure {hand}/{score}"
    return None
...
def cheap_postflop_continue(table, my_seat, base):
    ...
    if rank >= 2 and not champion.counter.patch1.fragile_rank_two(
        ...
        return "call", price, f"v002 cheap continue made rank {rank}"
    if opponents <= 3 and (top_pair or rank == 1):
        return "call", price, f"v002 cheap bluff catch rank {rank}"
    if draw and not texture.get("paired", False) and required <= 0.12:
        return "call", price, "v002 cheap draw continue"
    return None
...
def choose_action(table, my_seat) -> ActionDecision:
    ...
    return action, amount, f"2:{message}"
