...
_EXPLOIT_STATION_CALL_FREQ = 0.50
_EXPLOIT_STATION_FOLD_TO_BET = 0.40
_EXPLOIT_STATION_BET_FRACTION = 0.75
_EXPLOIT_BLUFFER_WASD = 0.45
_EXPLOIT_BLUFFER_AGGR = 0.35
_EXPLOIT_BLUFFCATCH_MAX_PRICE = 0.40  # required pot-odds ceiling for the call
...
def opponent_exploit_adjustment(table, my_seat, decision):
    ...
    if (
        ...
        target = _exploit_bet_amount(allowed, pot, _EXPLOIT_STATION_BET_FRACTION)
    ...
    if (
        ...
        required = pot_odds(price, max(pot, 1))
        if required <= _EXPLOIT_BLUFFCATCH_MAX_PRICE:
