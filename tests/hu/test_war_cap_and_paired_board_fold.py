...
def make_war_table(hole, board, pot, call, num_raises=3):
    ...
    for i in range(num_raises):
        history.append(
            {"agentId": HERO, "action": "raise", "amount": 40, "street": "Turn"}
        )
        history.append(
            {"agentId": "villain", "action": "raise", "amount": 80, "street": "Turn"}
        )
...
def test_war_cap_rank_3_not_affected():
    ...
    assert action == "raise", f"trips facing 4-bet should raise, got {action!r}: {msg}"
