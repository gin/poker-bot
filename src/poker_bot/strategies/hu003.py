"""
Poker strategy - hu003

Fix: Suppress flop value raises when trips are fully on the board
(board-dominated trips guard). On paired boards, the base strategy's
"Value raising" lookup sees made_hand_rank == 3 and raises, but when the
three-of-kind comes entirely from the board (e.g., T7 on Q-7-7), hero's
private cards contribute nothing. The hand is effectively one pair and
raising into a paired board is catastrophic.

Benchmark (10k hands, 1 seed, 18 opponents):
  All gates PASS (delta vs hubase within noise ±1.4 bb/100)
  Notable: +30.9 bb/100 vs s3v017, +16.5 vs s3v016, +12.7 vs s2v014
"""
