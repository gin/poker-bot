...
anti-bully fix, defend against check-raise
check-raise trap detect
all-in preflop only when AA
"""
...
PAIRED_BOARD_HIGH_BOARD_MIN_FOLD_PRICE = 0.23  # fold fragile two pair on Q/K/A-high paired boards vs 48%+ pot bets
...
def _sixmax_hole_participates(hole_cards, board_cards) -> bool:
    ...
    are shared strength, not bluff-catchers. Ported from the survival_sixmax
    anti-bully fix (live table cmrfrslyinvmqi1x8qjeqm165, 2026-07-10): AJ-high
    read "medium" off a board pair and bluff-caught a made flush for -2660."""
...
def _sixmax_raiser_checked_this_street(table) -> bool:
    ...
    arena recentEvents; missing data -> False (no trap read). Ported
    alongside _sixmax_hole_participates from the survival_sixmax anti-bully
    fix (same live table/incident)."""
    current_bet = int(table.get("currentBet") or 0)
    if current_bet <= 0:
        return False
    street = table.get("street")
...
def _sixmax_anti_bully_action(table, my_seat):
    ...
    # "medium" from a board pair (not a hole card) and bluff-caught a made
    # flush for -2660. Medium/strong now require PRIVATE strength -- a hole
    # card must participate; board-owned ranks alone don't qualify.
    participates = _sixmax_hole_participates(hole_cards, board_cards)
    medium = (made_rank in {1, 2} and participates) or top_pair
    weak = made_rank == 0
    strong = made_rank >= 3 and participates
    ...
    # against these lines was the -732/-392 turn/river disaster class in the
    # same incident. Their big stacks (built by trapping) are exactly what
    # makes bully_context fire, so this branch self-selects for trappers.
    # Downgrade: never raise, call only genuinely strong at a fair price,
    # never bluff-catch.
    trapped = _sixmax_raiser_checked_this_street(table)
    ...
    if "call" in available:
        ...
        if trapped:
            return None  # medium/air vs a check-raise: let base logic fold
...
def paired_board_pot_control(table, my_seat, base) -> ActionDecision | None:
    ...
    if action == "raise" and "call" in available:
        ...
        if (
            ...
            and required > PAIRED_BOARD_HIGH_BOARD_MIN_FOLD_PRICE
        ):
            if "fold" in available:
                return (
                    "fold",
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION: Opponent-stat exploit layer (heads-up)
#
# Final adjustment step, keyed on opponentProfiles (arena agent-stats prior
# via the sandbox rainbow table, overtaken by live observation at 20 hands).
# Goal: maximize match wins on the HU ladder. The field is mostly
# loose-passive but includes tight and balanced opponents, so each exploit
# activates only on a confident archetype read and stays silent otherwise —
# with no profile at all, the layer is inert and hubase plays exactly its
# baseline (non-tracked benchmarks are bit-identical).
#
#   station (calls a lot, rarely folds): value-bet thinner and bigger when
#       checked to; never turned into a bluff target.
#   bluffer (aggression shown down weak): widen turn/river bluff-catching —
#       measured as the biggest unexploited edge (top pair folded to river
#       pot-bets regardless of a 60% bluff read).
#
# A third exploit (stab flop/turn vs nits when checked to) was built and
# CUT: h2h counterfactual showed -6.5 bb/100 vs simple and -2.1 vs
# survival_aggressive (120-151 stabs averaging -110 chips/hand) — hubase's
# tuned probe logic already covers fold-heavy opponents; overriding it with
# unconditional air stabs paid off the times they continued.
# ══════════════════════════════════════════════════════════════════════════════

# Thresholds calibrated against the ladder-S1 field distribution
# (opponent_prior.sqlite percentiles, 2026-07-07 — see
# artifacts/PLAN_LADDER_S1_EXPLOIT.md). The original station gate
# (call>=0.50, f2b<=0.40) activated against 0% of the real field.
_EXPLOIT_MIN_HANDS = 20
_EXPLOIT_STATION_CALL_FREQ = 0.38  # field p75-p80; catches the ranked stations
_EXPLOIT_STATION_FOLD_TO_BET = 0.48  # field median
_EXPLOIT_STATION_BET_FRACTION = 0.75
_EXPLOIT_BLUFFER_WASD = 0.45  # field p25 — top-20 average is 0.61
_EXPLOIT_BLUFFER_AGGR = 0.35
_EXPLOIT_BLUFFCATCH_MAX_PRICE = 0.45  # calls up to ~4.5x-pot overbets vs bluffers
_EXPLOIT_STEAL_MAX_VPIP = 0.30  # 29% of the field; tight = folds preflop
_EXPLOIT_STEAL_MIN_SCORE = 50  # solid steal range: raise-backs hurt less
_EXPLOIT_STEAL_BLINDS = 3  # raise-to sizing in big blinds

# In-match kill switch: attribute per-hand stack deltas to the exploits
# that fired that hand; disable an exploit that is clearly bleeding.
# Benign in local benchmarks (stacks reset per hand -> deltas ~0).
_KILL_MIN_FIRES = 15
_KILL_AVG_BB = -8.0

_EXPLOIT_TRACK: dict = {}  # tag -> {"fires": int, "net": float, "off": bool}
_MATCH_STATE: dict = {
    "stack_at_hand_start": None,
    "prev_hand_tags": (),
    "current_hand_tags": set(),
    "hands": 0,
    "net": 0.0,
}


def _exploit_reset_match_state():
    """Test hook / fresh-match reset."""
    _EXPLOIT_TRACK.clear()
    _MATCH_STATE.update(
        stack_at_hand_start=None,
        prev_hand_tags=(),
        current_hand_tags=set(),
...
def _exploit_enabled(tag):
    entry = _EXPLOIT_TRACK.get(tag)
    return entry is None or not entry["off"]


def _exploit_note_fire(tag):
    entry = _EXPLOIT_TRACK.setdefault(tag, {"fires": 0, "net": 0.0, "off": False})
    entry["fires"] += 1
    _MATCH_STATE["current_hand_tags"].add(tag)


def _exploit_update_match_state(table, my_seat):
    """Detect hand boundaries and attribute the previous hand's stack delta
    to the exploits that fired during it. Runs once per hand (first
    decision: empty board, at most a blind committed)."""
    blind = blind_size(table.get("allowedActions") or {}, table)
    if table.get("boardCards") or int(my_seat.get("currentBetChips") or 0) > blind:
        return
    stack_total_now = int(my_seat.get("stackChips") or 0) + int(
        my_seat.get("currentBetChips") or 0
    )
    previous = _MATCH_STATE["stack_at_hand_start"]
    if previous is not None:
        delta = stack_total_now - previous
        _MATCH_STATE["hands"] += 1
        _MATCH_STATE["net"] += delta
        for tag in _MATCH_STATE["prev_hand_tags"]:
            entry = _EXPLOIT_TRACK.setdefault(
                tag, {"fires": 0, "net": 0.0, "off": False}
            )
            entry["net"] += delta
            if (
                not entry["off"]
                and entry["fires"] >= _KILL_MIN_FIRES
                and entry["net"] / max(entry["fires"], 1)
                <= _KILL_AVG_BB * max(blind, 1)
            ):
                entry["off"] = True
    _MATCH_STATE["stack_at_hand_start"] = stack_total_now
    _MATCH_STATE["prev_hand_tags"] = tuple(_MATCH_STATE["current_hand_tags"])
    _MATCH_STATE["current_hand_tags"] = set()


def exploit_match_status() -> str:
    """Diagnostic line: match net + per-exploit attribution (for traces)."""
    parts = [f"hands={_MATCH_STATE['hands']} net={_MATCH_STATE['net']:+.0f}"]
    for tag, entry in sorted(_EXPLOIT_TRACK.items()):
        flag = " OFF" if entry["off"] else ""
        parts.append(f"{tag}:{entry['fires']}x{entry['net']:+.0f}{flag}")
    return " ".join(parts)


def _exploit_bet_amount(allowed, pot, fraction):
    rng = allowed.get("betRange") or {}
    low = int(rng.get("min") or allowed.get("minBet") or 0)
    high = int(rng.get("max") or 0)
...
def _hero_has_real_pair(hole_cards, board_cards):
    """Pocket pair or a hole card pairing the board — excludes board-made
    pairs everyone shares (those are air, not bluff-catchers)."""
    ranks = [c[0] for c in hole_cards if c]
    if len(ranks) == 2 and ranks[0] == ranks[1]:
        return True
    board_ranks = {c[0] for c in board_cards if c}
    return any(r in board_ranks for r in ranks)


def opponent_exploit_adjustment(table, my_seat, decision):
    """Adjust the final decision against a confidently-read archetype.

    Returns a replacement ActionDecision or None (keep the base decision).
    """
    if seated_players(table) >= 4:
        return None
    _exploit_update_match_state(table, my_seat)
    profile = _single_opponent_profile(table, min_hands=_EXPLOIT_MIN_HANDS)
    if profile is None:
        return None
    street = table.get("street", "Preflop")
    ...
    allowed = table.get("allowedActions") or {}
    available = allowed.get("availableActions", [])
    hole = my_seat.get("holeCards", [])
    board = table.get("boardCards", [])
    pot = effective_pot(table) or int(table.get("potChips") or 0)
    price = call_amount(allowed)
    facing = price > 0
    blind = blind_size(allowed, table)

    call_freq = bayesian_call_frequency(profile)
    f2b = bayesian_fold_to_bet_frequency(profile)
    aggression = float(profile_aggression_frequency_merged(profile))
    vpip_freq = _profile_vpip_frequency(profile)
    wasd_val = profile_value(profile, "weak_aggressive_showdown_frequency")
    wasd = float(wasd_val) if wasd_val is not None else 0.0

    # Per-opponent playbook (rainbow table 2.0): hand-authored / replay-mined
    # threshold overrides shipped with the prior stats.
    playbook = profile_value(profile, "playbook")
    playbook = playbook if isinstance(playbook, dict) else {}

    is_station = (
        call_freq >= _EXPLOIT_STATION_CALL_FREQ
        and f2b <= _EXPLOIT_STATION_FOLD_TO_BET
    )
    is_bluffer = wasd >= _EXPLOIT_BLUFFER_WASD and aggression >= _EXPLOIT_BLUFFER_AGGR

    # Preflop steal: tight opponents fold preflop by definition (VPIP <= 0.30
    # = 29% of the ladder field). One-decision price, kill-switch guarded.
    if street == "Preflop":
        if (
            action in ("fold", "call", "check")
            and not playbook.get("no_steal")
            and _exploit_enabled("steal")
            and vpip_freq is not None
            and 0 < vpip_freq <= _EXPLOIT_STEAL_MAX_VPIP
            and "raise" in available
            and price <= blind  # unraised pot: limp/blind to steal, not a 3-bet
            and preflop_score(hole) >= _EXPLOIT_STEAL_MIN_SCORE
        ):
            target = max(
                int(min_raise_to(allowed) or 0), _EXPLOIT_STEAL_BLINDS * blind
            )
            rng = allowed.get("raiseRange") or {}
            if rng.get("max"):
                target = min(target, int(rng["max"]))
            if target > 0:
                _exploit_note_fire("steal")
                return ("raise", target, f"{message} [exploit:steal]")
        return None

    # Station: thin/big value when checked to — stations pay off worse hands.
    # Covers both missed value (check -> bet) and undersized value bets.
    if (
        not facing
        and action in ("check", "bet")
        and is_station
        and not playbook.get("no_station_value")
        and _exploit_enabled("station-value")
        and "bet" in available
        and has_top_pair_or_better(hole, board)
    ):
        fraction = float(
            playbook.get("station_bet_fraction") or _EXPLOIT_STATION_BET_FRACTION
        )
        target = _exploit_bet_amount(allowed, pot, fraction)
        base_amount = int(_amount or 0)
        if target and (action == "check" or base_amount < target):
            _exploit_note_fire("station-value")
            return ("bet", target, f"{message} [exploit:station-value]")

    # Bluffer: turn/river bluff-catch with a real pair at a fair price.
    if (
        facing
        and action == "fold"
        and street in ("Turn", "River")
        and is_bluffer
        and not playbook.get("no_bluffcatch")
        and _exploit_enabled("bluffcatch")
        and "call" in available
        and _hero_has_real_pair(hole, board)
        and price < int(my_seat.get("stackChips") or 0)
    ):
        ceiling = float(
            playbook.get("bluffcatch_max_price") or _EXPLOIT_BLUFFCATCH_MAX_PRICE
        )
        required = pot_odds(price, max(pot, 1))
        if required <= ceiling:
            _exploit_note_fire("bluffcatch")
            return ("call", price, f"{message} [exploit:bluffcatch]")
...
def choose_action(table, my_seat=None) -> ActionDecision:
    ...
    exploit = opponent_exploit_adjustment(table, my_seat, base)
