...
_DEFEND_RANGES = {
    ...
    "CO": _OPEN_RANGES["MP"] | {"77", "ATs", "KTs", "QTs", "JTs"},
    "BTN": _OPEN_RANGES["CO"] | {"55", "44", "33", "22", "A5s", "A4s", "K9s"},
    "SB": {"AA", "KK", "QQ", "JJ", "TT", "99", "AKs", "AKo", "AQs", "AJs", "KQs"},
    "BB": _OPEN_RANGES["BTN"] | {"K8s", "Q8s", "J8s", "T8s", "97s", "86s", "75s"},
}

_THREE_BET_RANGES = {
    position: {"AA", "KK", "QQ", "JJ", "AKs", "AKo", "AQs"} for position in _OPEN_RANGES
}
...
def _normalize_position(position):
    ...
    aliases = {
        "BUTTON": "BTN", "DEALER": "BTN", "LATE": "BTN",
        "MIDDLE": "MP", "EARLY": "UTG", "BLIND": "BB", "SHORT": "BTN",
    }
    return aliases.get(position, position if position in _OPEN_RANGES else "MP")
...
def default_preflop_range(position="MP", situation="open"):
    ...
    if situation in {"open", "unopened", "raise", "bet"}:
        return _HandRange.from_classes(_OPEN_RANGES[position])
    if situation in {"defend", "call", "facing_raise"}:
        return _HandRange.from_classes(_DEFEND_RANGES[position])
    if situation in {"three_bet", "3bet", "reraise"}:
        return _HandRange.from_classes(_THREE_BET_RANGES[position])
    if situation == "all":
...
def average_summary(summaries):
    ...
    averaged = {
        "tracker_strength": _average(s["posterior_strength"] for s in summaries),
        "tracker_range_advantage": _average(s["range_advantage"] for s in summaries),
        "tracker_bluff_frequency": _average(s["bluff_frequency"] for s in summaries),
        "tracker_value_frequency": _average(s["value_frequency"] for s in summaries),
        "tracker_capped_probability": _average(s["capped_probability"] for s in summaries),
        "tracker_confidence": _average(s["confidence"] for s in summaries),
