...
def preflop_open_raise(table, my_seat, base) -> ActionDecision | None:
    ...
    if score >= min_score:
        ...
        return (
            "raise",
            amount,
            f"v004 preflop open raise: score {score} pos {pos} size {size_bb}x",
        )
    return None
...
def preflop_positional_defense(table, my_seat, base) -> ActionDecision | None:
    ...
    return (
        "call",
        price,
        f"v004 preflop positional defense: score {score} pos {pos} required {required:.0%} cap {max_price:.0%}",
    )
...
def preflop_three_bet(table, my_seat, base) -> ActionDecision | None:
    ...
    if not should_three_bet and pos == "SB" and score >= 80:
        should_three_bet = True
    if not should_three_bet and pos == "SB" and hc in _3BET_BLUFF_SUITED:
        should_three_bet = True

    # --- BTN polar 3-bets vs CO/earlier ---
    if not should_three_bet and pos == "BTN" and score >= 82:
        should_three_bet = True
    if not should_three_bet and pos == "BTN" and hc in _3BET_BLUFF_SUITED:
        should_three_bet = True

    if not should_three_bet:
        return None

    # Sizing: 3x IP (BTN vs CO/earlier), 3.5x OOP, min 9 BB
    current_bet = int(table.get("currentBet") or 0)
    blind = blind_size(allowed, table)
    ip_positions = {"BTN"}
    multiplier = 3.0 if pos in ip_positions else 3.5
    target = max(BIG_BLIND * 9, int(current_bet * multiplier))
    amount = raise_to_amount(table, allowed, target)
    if amount is None:
        return None

    label = "value" if (hc in _3BET_VALUE_CLASSES or score >= 90) else "bluff"
    return (
        "raise",
        amount,
        f"v004 preflop 3-bet {label}: hand {hc} score {score} pos {pos} size {target}",
    )
...
def preflop_squeeze(table, my_seat, base) -> ActionDecision | None:
    ...
    # Probabilistic bluff squeeze from BTN/SB (30%)
    if not should_squeeze and pos in {"BTN", "SB"} and hc in {"A5s", "A4s", "KQs"}:
        chosen = choose_weighted(
            (("squeeze", 0.30), ("pass", 0.70)),
            "v004-preflop-bluff-squeeze",
            table,
            my_seat,
            strategy="flattened_v004",
    ...
    return (
        "raise",
        amount,
        f"v004 preflop squeeze: hand {hc} score {score} pos {pos} callers {callers} size {target}",
    )
...
def spr_commitment_lock(table, my_seat, base) -> ActionDecision | None:
    ...
    # stack = int(my_seat.get("stackChips") or 0) # bug: stack chips is not the effective stack
    stack = effective_pot(table)
    pot = int(table.get("potChips") or 0)
    spr = stack / max(1, pot)
...
def sixmax_adjustment(table, my_seat, base) -> ActionDecision | None:
    ...
    # 1. GTO preflop 3-bet vs a single open-raise (value + polar bluffs)
    decision = preflop_three_bet(table, my_seat, base)
    if decision is not None:
        return decision

    # 2. GTO preflop squeeze vs raise + callers
    decision = preflop_squeeze(table, my_seat, base)
    if decision is not None:
        return decision

    # 3. Proactive preflop open raising (GTO thresholds fixed in v4)
    ...
    # 4. Preflop positional defense (flat-call BTN/BB/CO only; OOP = 3-bet-or-fold)
    decision = preflop_positional_defense(table, my_seat, base)
    if decision is not None:
        return decision

    # 5. Tightly-gated postflop exploits. Each self-guards on table size and
    ...
    # 6. Universal fallbacks: reproduce flattened's exact cascade order so that
...
def choose_action(table, my_seat) -> ActionDecision:
    ...
    return action, amount, f"flattened_v004: {message}"
