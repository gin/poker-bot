...
Route by number of opponents at the table, and add:
1. Mathematical Commitment (SPR)
2. Positional & Textural Advantage (IP + Dry Board)
3. Opponent Tendencies (Fold/Call frequencies)
4. Table Dynamics (Exact opponent count)

Note:
To Do: improve on 6 player games. auto_research_v008 and v005 performing better
"""
...
def preflop_open_raise(table, my_seat, base) -> ActionDecision | None:
    ...
    # Position-aware minimum score thresholds for opening
    min_score = 99
    if pos in {"BTN", "CO", "SB"}:
        min_score = 55
    elif pos in {"HJ", "MP"}:
    ...
    if score >= min_score:
        amount = raise_to_amount(table, allowed, BIG_BLIND * 3.0)
        return (
            "raise",
            amount,
            f"v003 preflop open raise: score {score} pos {pos}",
        )
    return None
...
def preflop_positional_defense(table, my_seat, base) -> ActionDecision | None:
    """Defend/call wider based on position, but ONLY when facing a raise (no unopened limping)."""
    if table.get("street", "Preflop") != "Preflop":
        return None
    ...
    return (
        "call",
        price,
        f"v003 preflop positional defense: score {score} pos {pos} required {required:.0%} cap {max_price:.0%}",
    )
...
def sixmax_adjustment(table, my_seat, base) -> ActionDecision | None:
    ...
    # 1. Proactive preflop open raising (NEW)
    decision = preflop_open_raise(table, my_seat, base)
    if decision is not None:
        return decision

    # 1.5. Preflop isolation raise over limpers (NEW)
    decision = preflop_isolation_raise(table, my_seat, base)
    if decision is not None:
        return decision

    # 2. Preflop positional defense against raises (NEW, replaces loose_call/blind_defense)
    decision = preflop_positional_defense(table, my_seat, base)
    if decision is not None:
        return decision

    # 3. Tightly-gated exploits. Each self-guards on table size (true 2-handed)
    ...
    for exploit in (
        ...
        semi_bluff_exploit,  # NEW: aggressive betting with strong draws in position
    ):
        if (decision := exploit(table, my_seat, base)) is not None:
            return decision

    # 4. Universal fallbacks: reproduce flattened's exact cascade order so that
    # v3 never gives back EV in spots the specialized branches did not handle.
    for adjustment in (
...
def choose_action(table, my_seat) -> ActionDecision:
    ...
    return action, amount, f"flattened_v003: {message}"
