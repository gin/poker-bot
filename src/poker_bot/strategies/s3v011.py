...
Optimize for heads up (1 opponent table)
    ...
    Opponent                   s3base  s3v010  Delta  Gate
    ─────────────────────────  ──────  ──────  ─────  ───────
 ...
 Arena analysis (telemetry-luigi-playground.sqlite, s3v005 run, 58 hands):
 - 48.3% win rate, net -2,371 chips (-20.5 bb/100)
 - Qd Kc preflop min-raise war: -1,753 (58% of total loss)
 - Ks 5d postflop min-raise war: -684 (23% of total loss)
 - Without these two hands, bot would be +66 (+1.1 bb/100)
 - The preflop_min_raise_war_cap guard addresses the Qd Kc case
 - Postflop min-raise wars (Ks 5d) remain unaddressed — extending the guard
   to postflop made benchmark regress, so it's reverted
"""
