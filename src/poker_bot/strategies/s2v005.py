...
Changes:
    postflop_draw_continue (s2base.py:2921-2997) — a new fallback that converts folds to calls when:                                        │
                                                                                                                                            │
    • Hero has a flush draw or OESD                                                                                                         │
    • required is below a street- and opponent-count-dependent cap (flop 0.30 HU / 0.22 multiway, turn 0.20 HU / 0.15 multiway)             │
    • price <= 20% of stack (no over-commit)                                                                                                │
                                                                                                                                            │
    Strictly additive — only converts base folds to calls. Reuses has_flush_draw and has_open_ended_straight_draw which already exist.      │
                                                                                                                                            │
    Verification                                                                                                                            │
                                                                                                                                            │
      Hero hand  Board   Draw               Before  After                                                                                   │
      ─────────  ──────  ─────────────────  ──────  ─────────────────                                                                       │
      6h5h       Th9h2s  FD + gutshot       fold    call ✓                                                                                  │
      8h7h       Ah5h2c  FD                 fold    call ✓                                                                                  │
      7h6h       Th8h3c  FD                 fold    call ✓                                                                                  │
      9h8d       JhTc5s  OESD               fold    call ✓                                                                                  │
      KhQc       Th9h2s  K-high only        fold    fold (unchanged)                                                                        │
      AdKd       Th9h2s  A-high, BDFD only  fold    fold (unchanged)                                                                        │
      Th9h       JhTc5s  bottom pair        call    call (unchanged)                                                                        │
      JhTc       JhTc5s  top two pair       raise   raise (unchanged)                                                                       │
                                                                                                                                            │
    4 strong-draw scenarios that were folding now call. 5 non-draw scenarios unchanged.                                                     │
                                                                                                                                            │
    Benchmark across 8 seeds × 3 opponents                                                                                                  │
                                                                                                                                            │
      Opponent           Avg delta (bb/100)  Range                                                                                          │
      ─────────────────  ──────────────────  ─────────────                                                                                  │
      adaptive           +2.1                +0.3 to +7.0                                                                                   │
      simple             +9.8                +6.7 to +13.3                                                                                  │
      survival_balanced  +0.4                -1.0 to +2.0

    All three opponent pools show positive EV on average. The vs simple gain is largest (9.8 bb/100) because simple c-bets a tight range    │
    and the bot was throwing away 30%+ equity on every draw.                                                                                │
                                                                                                                                            │
    pytest: 452/452 pass.                                                                                                                   │
                                                                                                                                            │
    Why this fix works when preflop widening didn't                                                                                         │
                                                                                                                                            │
    Draws realise their raw equity without needing postflop play — either you hit the draw or you check-fold. So calling with a draw is     │
    +EV regardless of how weak the bot's postflop turn play is. The preflop widenings failed because they required the bot to play OOP      │
    postflop (with made hands and weak draws), where the realisation gap killed the EV. Draws bypass that.
...
def anti_threshold_choose_action(table, my_seat):
    ...
    if street == "Preflop":
        ...
        if "raise" in available and score >= raise_threshold:
            amount = raise_for_value(table, allowed, strong=score >= premium_threshold)
