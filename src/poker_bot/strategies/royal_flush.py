"""Balanced survival strategy with royal-flush potential protection.

Same base as survival_balanced_pp_pd_postflop_raise, with patch 1:
rank-2 made hands are checked against board texture before value betting,
raising, or defending. Board-made two pair and paired-board bluff catchers are
not treated as automatic value hands.

Royal-flush potential is protected from folds: check when free, raise when
allowed, and call only when facing action without a legal raise.
"""
...
def royal_flush_possible(hole_cards, board_cards):
...
def royal_flush_override(table, my_seat):
    ...
    if "check" in available:
        return "check", None, "royal flush possible, checking"

    if "raise" in available:
        if table.get("street", "Preflop") == "Preflop":
            amount = balanced_raise_amount(table, allowed, preflop_score(hole_cards))
        else:
            amount = postflop_raise_amount(table, allowed, semi_bluff=True)
        if amount is None:
            raise_range = allowed.get("raiseRange") or {}
            amount = allowed.get("minRaiseTo") or raise_range.get("min")
        return "raise", amount, "royal flush possible, raising"

    if "call" in available:
        call_amount = int(allowed.get("callAmount") or allowed.get("callChips") or 0)
        return "call", call_amount, "royal flush possible, calling without raise"
...
def postflop_raise_back_defense(table, my_seat, blueprint):
    ...
    available = allowed.get("availableActions", [])
    call_amount = int(allowed.get("callAmount") or allowed.get("callChips") or 0)
    if call_amount <= 0 or "call" not in available:
        return None
    ...
    if rank >= 3:
        ...
        if required <= 0.45:
            return "call", call_amount, f"postflop strong call rank {rank}"

    if board_dominated_two_pair(hole_cards, board_cards, rank):
        if required <= 0.16:
            return "call", call_amount, "board-dominated two-pair bluff catch"
        if "fold" in available:
    ...
    if rank == 2:
        if paired_board_rank_two(hole_cards, board_cards, rank):
            if required <= 0.28:
                return "call", call_amount, "paired-board rank-2 pot control"
            if "fold" in available:
        ...
        if required <= 0.40:
            return "call", call_amount, "postflop strong pair/trips defense rank 2"
    ...
    if medium:
        ...
        if required <= 0.26:
            return "call", call_amount, f"postflop medium defense rank {rank}"
    ...
    if draw and street in {"Flop", "Turn"}:
        ...
        if required <= 0.22:
            return "call", call_amount, "postflop draw defense"
...
def choose_action(table, my_seat) -> ActionDecision:
    ...
    royal_flush = royal_flush_override(table, my_seat)
