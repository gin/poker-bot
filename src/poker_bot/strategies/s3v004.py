...
Optimize for heads up (1 opponent table)
    ...
    Opponent                   s3base  s3v003  Delta  Gate
    ─────────────────────────  ──────  ──────  ─────  ───────
 ...
 1. Added river_two_pair_raise_guard(): prevents value-raising with two pair
    on the river against tight opponents (VPIP% < 25%). Converts raise/bet →
    check/call. Two pair on the river is often beaten by sets, straights,
    flushes, or better two pair. Against a tight opponent's condensed river
    betting range, the hand is a bluff-catcher at best.

    The guard is inert against the 18 benchmark opponents (none qualify as
    tight under the 25% VPIP threshold), so benchmark deltas are +0.0.
    Validated by tests/hu/test_river_two_pair_raise.py (12 tests).

    Reference: PLAN_FIX_RIVER_2P.md
"""
