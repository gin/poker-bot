"""
Poker strategy - hu004

Fix 1: Postflop war cap — fold rank 2 (two pair) when facing 4-bet at
>33% pot odds, instead of always calling. The old guard converted raise→call
for all rank < 3 hands, but two pair at -33% equity vs a 4-bet range should
fold when the price is too high.

Fix 2: Full house positive control for 77 — the medium_pair_paired_board_fold
guard was incorrectly folding 77 on paired boards even when hero had a full
house (e.g., 77 on 3-3-7 = 77733). Added evaluate_hand check: if rank >= 5,
allow the hand to continue.

Benchmark (10k hands, 1 seed, 17 opponents):
  profiled_counter_adaptive: +32.2 (was -15.8k before patch — +48k swing)
  All other opponents: positive
"""
