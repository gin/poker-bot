"""
Flattened v1 — Single-file strategy.

Preflop logic is copied exactly from auto_research_v007's dependency chain
(profiled_counter_adaptive, survival_sixmax, survival_balanced_pp_pd_pr_patch1)
to guarantee 100% identical behavior without simplification.
Postflop logic is delegated directly to auto_research_v007.
...
def sixmax_adjustment(table, my_seat, base) -> ActionDecision | None:
    ...
    # 1. Multi-way specific (4+ opponents): Prioritize extreme defense and pot control
    if opponents >= 4:
        for adjustment in (
            paired_board_range_fold,  # 1. Fold weak hands on dangerous paired boards
            paired_board_pot_control,  # 2. Check/fold fragile hands instead of betting/raising
            strong_top_pair_defense,  # 3. Only call with strong top pair at strict price caps
            simple_profile_river_bluff_catch,  # 4. River-specific: catch bluffs from value-heavy opponents
        ):
            if (decision := adjustment(table, my_seat, base)) is not None:
                return decision

    # 2. Short-handed specific (2-3 opponents): Prioritize aggression and pressure
    elif opponents <= 3:
        for adjustment in (
            preflop_premium_pressure,
            range_mixed_dry_probe,
            cheap_postflop_continue,
            strong_top_pair_defense,  # Still good to defend top pair
        ):
            if (decision := adjustment(table, my_seat, base)) is not None:
                return decision

    # 3. Universal fallbacks (apply to all table sizes if nothing else triggered)
    for adjustment in (
        preflop_loose_call,
...
def choose_action(table, my_seat) -> ActionDecision:
    ...
    return action, amount, f"flattened_v001: {message}"
