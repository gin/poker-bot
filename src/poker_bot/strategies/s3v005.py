...
Optimize for heads up (1 opponent table)
    ...
    Opponent                   s3base  s3v004  Delta  Gate
    ─────────────────────────  ──────  ──────  ─────  ───────
 ...
 1. Added preflop_min_raise_war_cap(): caps preflop raises after 3+ raise-backs
    in the same hand. Converts raise → call/check. Prevents min-raise wars where
    the bot and opponent keep min-raising each other preflop, bloating the pot
    without changing the outcome.

    The guard is inert against the 18 benchmark opponents (none trigger
    min-raise wars under the 3-raise-back threshold in benchmark conditions),
    so benchmark deltas are +0.0.
    Validated by tests/hu/test_preflop_war_cap.py (12 tests).

    Reference: PLAN_FIX_PREFLOP_WAR.md
"""
