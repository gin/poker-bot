...
Optimize for heads up (1 opponent table)
    ...
    Opponent                   s3base  s3v007  Delta  Gate
    ─────────────────────────  ──────  ──────  ─────  ───────
 ...
 2. Added river_two_pair_raise_guard(): prevents value-raising with two pair
    on the river vs tight opponents. Two pair on the river is often beaten by
    sets, straights, flushes, or better two pair.

 3. Added preflop_min_raise_war_cap(): caps preflop raises after 3+ raise-backs
    in the same hand. Converts raise → call/check. Prevents min-raise wars.
 ...
 Note: A preflop_score_42_fold guard was attempted but reverted because it
 made benchmark performance worse (-1.5 bb/100 per opponent). The score 42
 leak (-15.7k per 50k hands) remains unaddressed.
"""
