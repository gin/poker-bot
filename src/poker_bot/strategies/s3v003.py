...
Optimize for heads up (1 opponent table)
    ...
    Opponent                   s3base  s3v002  Delta  Gate
    ─────────────────────────  ──────  ──────  ─────  ───────
    simple                     +24.7   +24.7   +0.0   ✅ PASS
    ...
    anti_threshold             +27.7   +27.7   +0.0   ✅ PASS
    royal_flush                +10.4   +10.4   +0.0   ✅ PASS
    royal_adaptive             +10.4   +10.4   +0.0   ✅ PASS
    survival_balanced          +21.7   +21.7   +0.0   ✅ PASS
    survival_aggressive        +39.7   +39.7   +0.0   ✅ PASS
    auto_research_v005         +16.3   +16.3   +0.0   ✅ PASS
    auto_research_v008         +16.3   +16.3   +0.0   ✅ PASS
    flattened_v2               +13.7   +13.7   +0.0   ✅ PASS
    s2baseog                   +14.3   +14.3   +0.0   ✅ PASS
    s2v002                     +14.6   +14.6   +0.0   ✅ PASS
    s2v004                     +14.3   +14.3   +0.0   ✅ PASS
    s2v008                     +14.2   +14.2   +0.0   ✅ PASS
    s2v009                     +6.7    +6.7    +0.0   ✅ PASS
    s2v014                     +6.6    +6.6    +0.0   ✅ PASS

 Changes made to s3base.py

 1. Added board_assisted_two_pair_guard(): prevents over-valuing two pair on
    a paired board when one of the pair ranks is fully on the board. Fires
    only when opponent profile shows VPIP < 25% (postflop-tight). Converts
    raise/bet → check/call and call → fold at >25% pot odds.

 2. Initial implementation had a bug: the raise→check branch did not
    require is_tight, so it fired against wide-range benchmark opponents
    (s2v002, s2baseog, flattened_v2, etc.) losing 73-100 chips per guarded
    hand. Tightened to require is_tight on all branches — guard is now
    inert against the 18 benchmark opponents (none qualify as tight under
    the 25% VPIP threshold) and behaviorally identical to s3v002 in
    benchmark conditions.

    The fix is validated by tests/hu/test_two_pair_paired_board_raise.py
    (8 tests) against the original telemetry hand cmqmv6leork81f6jtmytkgs2r
    where the bot lost 1015 chips to a tight opponent. Benchmark cannot
    validate the fix because the benchmark opponent lineup does not
    include a profile that matches the original leak (tight VPIP < 25%).

 Reference: PLAN_FIX_BOARD_PAIR.md
"""
