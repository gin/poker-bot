...
Optimize for heads up (1 opponent table)
    ...
    Opponent                   s3base  s3v005  Delta  Gate
    ─────────────────────────  ──────  ──────  ─────  ───────
 ...
 1. Added preflop_min_raise_war_cap(): caps preflop raises after 3+ raise-backs
    ...
    the bot and opponent keep min-raising each other preflop.

 2. Modified simulator.py (run_betting_round, run_betting_round_multiway) to
    track per-round action history and inject it into the table dict as
    actionHistory before each strategy call. This enables guards to count
    raise-backs and detect min-raise wars.
