"""Balanced survival strategy with wider preflop defense + postflop raise-backs.

Same base as survival_balanced_pp_preflop_defense, with an added postflop
facing-bet layer so strong hands, dry-board medium hands, and real draws are not
forced through passive call/fold fallback lines.
"""
...
def postflop_raise_back_defense(table, my_seat, blueprint):
    ...
    if rank == 2:
        if can_raise and street in {"Flop", "Turn"} and required <= 0.36:
